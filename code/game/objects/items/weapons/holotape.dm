...
/obj/item/tapeproj
	...
	var/turf/start
	var/turf/end
	var/tape_type = /obj/item/tapeproj
	var/icon_base
	var/charging = 0
...
/obj/item/tapeproj/security
	...
	tape_type = /obj/item/tapeproj/security
	icon_base = "security"
...
/obj/item/tapeproj/engineering
	...
	tape_type = /obj/item/tapeproj/engineering
...
/obj/item/tapeproj/attack_self(var/mob/user)
	...
	if(icon_state == "[icon_base]_start")
		...
		while (cur!=end && can_place)
			if(cur.density == 1)
				...
				for(var/obj/O in cur)
					if(!istype(O, /obj/item/tapeproj) && O.density)
						can_place = 0
						break
		...
		var/tapetest = 0
		while (cur!=end)
			for(var/obj/item/tapeproj/Ptest in cur)
				if(Ptest.icon_state == "[Ptest.icon_base]_[dir]")
					tapetest = 1
			if(tapetest != 1)
				var/obj/item/tapeproj/P = new tape_type(cur)
...
/obj/item/tapeproj/Bumped(var/mob/M)
	if(!ismob(M))
		return
	if(iscarbon(M))
...
/obj/item/tapeproj/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	if(!density) return 1
...
/obj/item/tapeproj/attack_hand(mob/living/user)
...
/obj/item/tapeproj/proc/healthcheck()
	if(health <= 0)
		breaktape()

/obj/item/tapeproj/ex_act(severity, target)
	breaktape()

/obj/item/tapeproj/blob_act()
	breaktape()

/obj/item/tapeproj/attack_paw(mob/living/user)
	attack_hand(user)

/obj/item/tapeproj/bullet_act(var/obj/item/projectile/Proj)
...
/obj/item/tapeproj/attackby(obj/item/weapon/W as obj, mob/user as mob, params)
	user.changeNext_move(CLICK_CD_MELEE)
...
/obj/item/tapeproj/hitby(AM as mob|obj)
	..()
	var/tforce = 0
	if(ismob(AM))
		tforce = 5
...
/obj/item/tapeproj/proc/breaktape()
	var/dir[2]
	...
	for(var/i=1;i<3;i++)
		...
		while(N != 1)
			N = 1
			for (var/obj/item/tapeproj/P in cur)
